...
export class U {
  ...
  static measurableGetArrays(measureHtml: HTMLElement | SVGElement, e: Event): {rules: Attr[], imports: Attr[], exports: Attr[], variables: Attr[],
    ...
    if (!measureHtml) { measureHtml = e.currentTarget as HTMLElement | SVGElement; }
    ...
    if (!measureHtml || measureHtml as any === document) {
      ...
      while (!measureHtml.classList.contains('measurable')) { measureHtml = measureHtml.parentElement; console.log('measureHtml3:', measureHtml); } }
...
// Prevent the backspace key from navigating back.
  ...
  static computeMeasurableAttributeRightPart(str: string, attr: Attr, logic: ModelPiece, measurableHtml: HTMLElement | SVGElement,
                                             size: Size = null, absTargetSize: Size = null, relTargetSize: Size = null): any {
    ...
    if (!absTargetSize) { absTargetSize = U.sizeof(document.body); }
    ...
    try { str = eval(str); } catch (e) { U.pw(true, 'error occurred while evaluating ', str, 'in measurable attribute ', attr, 'err:', e); }
    return str; }
  ...
  static processMeasurableChain(attr0: Attr, logic: ModelPiece, measurableHtml: HTMLElement | SVGElement,
    ...
    const destination: string = attr0.value;
    const attributePseudoSelector = '->';
  ...
  static processMeasurableDstyle(attr: Attr, logic: ModelPiece, html: HTMLElement | SVGElement, size: Size = null, absTargetSize: Size = null): void {
    ...
    const elem: HTMLElement | SVGElement = attr.parentElement;
    const fake: HTMLElement = document.createElement('div');
