import {
  ...
  ShortAttribETypes,
  Status, EEnum,
  U, Dictionary, AttribETypes, IAttribute, PropertyBarr, M2Package, IEdge, EParameter, IClassChild
} from '../../common/Joiner';
...
export class Type {
  ...
  static updateAllMMClassSelectors() {
  ...
  private static makeTypeSelector(selectHtml: HTMLSelectElement, selectedType: Type, addPrimitive: boolean, addEnum: boolean, addClass: boolean, addVoid: boolean): void {
    ...
    U.clear(selectHtml);
    const mp: ModelPiece = ModelPiece.getLogic(selectHtml) as IClassChild;
    if (mp && mp instanceof IClassChild) {
