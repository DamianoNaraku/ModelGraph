...
import {
  ...
  EdgeModes, EOperation, EParameter
} from '../../common/Joiner';
...
export class StyleEditor {
  ...
  showC(m: IClass) {
    ...
    // todo: non possibile per instancePreview, l'oggetto da disegnare non esiste / non è specificato.
    ...
    const onStyleChange = (evt: ChangeEvent | BlurEvent | KeyDownEvent | ClickEvent) => {
      if (htmlInput.innerText === 'invisible' || (htmlInput.innerText !== '' && htmlInput.innerText.trim() === '')) {
        htmlInput.innerText = '<span></span>'; }
      if (!U.isValidHtml(htmlInput.innerText)) {
        U.pw(true, 'invalid html, check the preview');
        htmlPreview.innerHTML = 'The html input is invalid, validate it at <a href="https://validator.w3.org/#validate_by_input" target="_blank">' +
          'validator.w3.org</a> or try&nbsp;';
        const a: HTMLAnchorElement = U.toHtml<HTMLAnchorElement>('<a class="autofix" href="#">autofix</a>');
        htmlPreview.appendChild(a);
        $(a).on('click', (evt2: ClickEvent) => {
          const tmp: HTMLElement = document.createElement('div');
          tmp.innerHTML = htmlInput.innerText;
          htmlInput.innerText = U.replaceAll((tmp.firstChild as HTMLElement | SVGElement).outerHTML, '\n', ' ');
          onStyleChange(evt2);
        });
        return; }
      htmlInput.innerText = U.changeVarTemplateDelimitersInMeasurables(htmlInput.innerText, '$', '£');
      ...
      if ((evt as any).isTrigger) { return; }
      m.refreshGUI(); };

    const onStyleChangeI = (e: ChangeEvent | BlurEvent | KeyDownEvent) => {
      if (htmlInputI.innerText === 'invisible' || (htmlInputI.innerText !== '' && htmlInputI.innerText.trim() === '')) {
        htmlInputI.innerText = '<span></span>'; }
      if (! U.isValidHtml(htmlInputI.innerText)) {
        htmlPreviewI.innerHTML = 'The html input is invalid, validate it at <a href="https://validator.w3.org/#validate_by_input">' +
          'validator.w3.org</a>';
        return; }
      htmlInputI.innerText = U.changeVarTemplateDelimitersInMeasurables(htmlInputI.innerText, '$', '£');
  ...
  showA(m: IAttribute) {
    ...
    const onStyleChange = (e: ChangeEvent | BlurEvent | KeyDownEvent) => {
      if (! U.isValidHtml(htmlInput.innerText)) {
        htmlPreview.innerHTML = 'The html input is invalid, validate it at <a href="https://validator.w3.org/#validate_by_input">' +
          'validator.w3.org</a>';
        return; }
      htmlInput.innerText = U.changeVarTemplateDelimitersInMeasurables(htmlInput.innerText, '$', '£');
      htmlPreview.innerHTML = htmlInput.innerText;
    ...
    const onStyleChangeI = (e: ChangeEvent | BlurEvent | KeyDownEvent) => {
      if (! U.isValidHtml(htmlInputI.innerText)) {
        htmlPreviewI.innerHTML = 'The html input is invalid, validate it at <a href="https://validator.w3.org/#validate_by_input">' +
          'https:https://validator.w3.org/#validate_by_input</a>';
        return; }
      htmlInputI.innerText = U.changeVarTemplateDelimitersInMeasurables(htmlInputI.innerText, '$', '£');
      htmlPreviewI.innerHTML = htmlInputI.innerText;
      m.styleOfInstances = U.toHtml(htmlInputI.innerText);
  ...
  showO(m: EOperation) {
    console.log('styleShowO(', m, ')');
    const model: IModel = m.getModelRoot();
    const html: HTMLElement = this.getCopyOfTemplate(m, '.operation');
    const $html = $(html);
    const htmlInput: HTMLTextAreaElement | HTMLDivElement
      = $html.find('.own.operationStyle.html')[0] as HTMLTextAreaElement | HTMLDivElement;
    const htmlPreview: HTMLElement = $html.find('.operationStyle.own.preview')[0] as HTMLElement;
    const htmlInputI: HTMLElement = $html.find('.operationStyle.instances.html')[0] as HTMLElement;
    const htmlPreviewI: HTMLElement = $html.find('.operationStyle.instances.preview')[0] as HTMLElement;
    htmlInput.setAttribute('placeholder', U.replaceVarsString(m, htmlInput.getAttribute('placeholder')));
    htmlInput.innerText = (m.getStyle() as HTMLElement).outerHTML;

    if (model.isMM()) {
      htmlInputI.setAttribute('placeholder', U.replaceVarsString(m, htmlInputI.getAttribute('placeholder')));
      htmlInputI.innerText =
        m.styleOfInstances ? m.styleOfInstances.outerHTML : ModelPiece.GetStyle(Status.status.m, 'EOperation').outerHTML;
    }
    ...
    $(htmlInput).off('paste.set').on('paste.set', StyleEditor.onPaste)
      .off('change.set').on('change.set', onStyleChange)
      .off('blur.set').on('blur.set', onStyleChange)
      .off('keydown.set').on('keydown.set', (e: KeyDownEvent) => { if (e.key === 'Esc') { this.showO(m); } } ).trigger('change');
    $(htmlInputI).off('paste.set').on('paste.set', StyleEditor.onPaste)
      .off('change.set').on('change.set', onStyleChangeI)
      .off('blur.set').on('blur.set', onStyleChangeI)
      .off('keydown.set').on('keydown.set', (e: KeyDownEvent) => { if (e.key === 'Esc') { this.showO(m); } } ).trigger('change');
  ...
  showParam(m: EParameter) {
    console.log('styleShowParam(', m, ')');
    const model: IModel = m.parent ? m.getModelRoot() : Status.status.getActiveModel();
    const html: HTMLElement = this.getCopyOfTemplate(m, '.parameter');
    const $html = $(html);
    const htmlInput: HTMLTextAreaElement | HTMLDivElement =
      $html.find('.own.parameterStyle.html')[0] as HTMLTextAreaElement | HTMLDivElement;
    const htmlPreview: HTMLElement = $html.find('.parameterStyle.own.preview')[0] as HTMLElement;
    const htmlPreviewI: HTMLElement = $html.find('.parameterStyle.instances.preview')[0] as HTMLElement;
    const htmlInputI: HTMLElement = $html.find('.parameterStyle.instances.html')[0] as HTMLElement;
    htmlInput.setAttribute('placeholder', U.replaceVarsString(m, htmlInput.getAttribute('placeholder')));
    htmlInput.innerText = m.getStyle().outerHTML;
    if (model.isMM()) {
      htmlInputI.setAttribute('placeholder', U.replaceVarsString(m, htmlInputI.getAttribute('placeholder')));
      htmlInputI.innerText =
        m.styleOfInstances ? m.styleOfInstances.outerHTML : ModelPiece.GetStyle(Status.status.m, 'EParameter').outerHTML;
    }
    ...
    $(htmlInput).off('paste.set').on('paste.set', StyleEditor.onPaste)
      .off('change.set').on('change.set', onStyleChange)
      .off('blur.set').on('blur.set', onStyleChange)
      .off('keydown.set').on('keydown.set', (e: KeyDownEvent) => { if (e.key === 'Esc') { this.showParam(m); } } ).trigger('change');
    $(htmlInputI).off('paste.set').on('paste.set', StyleEditor.onPaste)
      .off('change.set').on('change.set', onStyleChangeI)
      .off('blur.set').on('blur.set', onStyleChangeI)
      .off('keydown.set').on('keydown.set', (e: KeyDownEvent) => { if (e.key === 'Esc') { this.showParam(m); } } ).trigger('change');
