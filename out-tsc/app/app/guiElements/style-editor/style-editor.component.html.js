/** @class */ (function () {
    function class_1() {
    }
    return class_1;
}());
"styleContainer" >
    /** @class */ (function () {
        function class_2() {
        }
        return class_2;
    }());
"StyleEditorDisplay" >
    /div>
    < div;
var default_1 = /** @class */ (function () {
    function default_1() {
    }
    return default_1;
}());
"styleTemplates styleEditor" >
    /** @class */ (function () {
        function class_3() {
        }
        return class_3;
    }());
"Template model" >
    Grid < /td>
    < td > X;
type;
"number";
var default_2 = /** @class */ (function () {
    function default_2() {
    }
    return default_2;
}());
"gridX";
min = "0";
step = "1";
placeholder = "off" > Y;
type;
"number";
var default_3 = /** @class */ (function () {
    function default_3() {
    }
    return default_3;
}());
"gridY";
min = "0";
step = "1";
placeholder = "off" > /td>
    < /tr>
    < tr >
    Zoom < /td>
    < td > X;
type;
"number";
var default_4 = /** @class */ (function () {
    function default_4() {
    }
    return default_4;
}());
"zoomX";
min = "0";
step = "0.01";
placeholder = "1" > Y;
type;
"number";
var default_5 = /** @class */ (function () {
    function default_5() {
    }
    return default_5;
}());
"zoomY";
min = "0";
step = "0.01";
placeholder = "1" > /td>
    < /tr>
    < tr > colspan;
"2" > type;
"checkbox";
var default_6 = /** @class */ (function () {
    function default_6() {
    }
    return default_6;
}());
"showGrid" > Show;
grid < /label></td > /tr>
    < tr > colspan;
"2" > type;
"color";
var default_7 = /** @class */ (function () {
    function default_7() {
    }
    return default_7;
}());
"graphColor" > Background;
color < /label></td > /tr>
    < /tbody></table >
    /div>
    < div;
var default_8 = /** @class */ (function () {
    function default_8() {
    }
    return default_8;
}());
"Template modelpiece" >
    --https;
/** @class */ (function () {
    function class_4() {
    }
    return class_4;
}());
"arrowContainer" >
    /** @class */ (function () {
        function class_5() {
        }
        return class_5;
    }());
"arrow up btn btn-primary" > ;
/button>
    < h6;
var default_9 = /** @class */ (function () {
    function default_9() {
    }
    return default_9;
}());
"tsclass" > TS_Class < /h6>
    < button;
var default_10 = /** @class */ (function () {
    function default_10() {
    }
    return default_10;
}());
"arrow down btn btn-primary" > ;
/button>
    < /div>
    < label;
var default_11 = /** @class */ (function () {
    function default_11() {
    }
    return default_11;
}());
"m1 iclass" >
    type;
"checkbox";
var default_12 = /** @class */ (function () {
    function default_12() {
    }
    return default_12;
}());
"showAsEdge" >
    /** @class */ (function () {
        function class_6() {
        }
        return class_6;
    }());
"showAsEdgeText" > ;
/span>
    < /label>
    < div;
var default_13 = /** @class */ (function () {
    function default_13() {
    }
    return default_13;
}());
"style own colContainer" >
    /** @class */ (function () {
        function class_7() {
        }
        return class_7;
    }());
"separatorShell top" >
    /** @class */ (function () {
        function class_8() {
        }
        return class_8;
    }());
"colChild" >
    /** @class */ (function () {
        function class_9() {
        }
        return class_9;
    }());
"roww" >
    /** @class */ (function () {
        function class_10() {
        }
        return class_10;
    }());
"vertexStyle separator htmllevel" > /h6>
    < /div>
    < div;
var default_14 = /** @class */ (function () {
    function default_14() {
    }
    return default_14;
}());
"colChild" >
    /** @class */ (function () {
        function class_11() {
        }
        return class_11;
    }());
"stylelabel" > Applied;
view: /span>
    < select;
var default_15 = /** @class */ (function () {
    function default_15() {
    }
    return default_15;
}());
"stylename" > /select>
    < /div>
    < /label>
    < button;
var default_16 = /** @class */ (function () {
    function default_16() {
    }
    return default_16;
}());
"helpButton btn btn-info";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < label : ;
var default_17 = /** @class */ (function () {
    function default_17() {
    }
    return default_17;
}());
"allowEdit" > This;
style;
is;
being;
inherited;
and;
cannot;
be;
modified;
here. < br > You;
need;
a;
local;
copy;
to;
allow;
Edit. < button;
var default_18 = /** @class */ (function () {
    function default_18() {
    }
    return default_18;
}());
"allowEdit btn btn-secondary" > Make;
it < /button></label >
    contenteditable;
"true";
var default_19 = /** @class */ (function () {
    function default_19() {
    }
    return default_19;
}());
"html";
placeholder = "Html for $##name$ (m2 class)." > /div>
    < !--;
-- >
    /** @class */ (function () {
        function class_12() {
        }
        return class_12;
    }());
"roww nowrap" >
    target;
"div.detail.measurable";
var default_20 = /** @class */ (function () {
    function default_20() {
    }
    return default_20;
}());
"detail measurable btn btn-primary" >
    /** @class */ (function () {
        function class_13() {
        }
        return class_13;
    }());
"closed" >
    /** @class */ (function () {
        function class_14() {
        }
        return class_14;
    }());
"measurable" >
    type;
"checkbox" >
    Measurable < /span>
    < /label>
    < div;
var default_21 = /** @class */ (function () {
    function default_21() {
    }
    return default_21;
}());
"HorizontalContainer" > /** @class */ (function () {
    function class_15() {
    }
    return class_15;
}());
"HorizontalChild" > ;
/span><span class="HorizontalChild">⮟</span > /** @class */ (function () {
    function class_16() {
    }
    return class_16;
}());
"HorizontalChild" > ;
/span></div >
    /span>
    < span;
var default_22 = /** @class */ (function () {
    function default_22() {
    }
    return default_22;
}());
"opened" >
    /** @class */ (function () {
        function class_17() {
        }
        return class_17;
    }());
"measurable" >
    /label>
    < div;
var default_23 = /** @class */ (function () {
    function default_23() {
    }
    return default_23;
}());
"HorizontalContainer" > /** @class */ (function () {
    function class_18() {
    }
    return class_18;
}());
"HorizontalChild" > ;
/span><span class="HorizontalChild">⮝</span > /** @class */ (function () {
    function class_19() {
    }
    return class_19;
}());
"HorizontalChild" > ;
/span></div >
    /span>
    < /button>
    < button;
target = "div.detail.save";
var default_24 = /** @class */ (function () {
    function default_24() {
    }
    return default_24;
}());
"detail save btn btn-primary" >
    /** @class */ (function () {
        function class_20() {
        }
        return class_20;
    }());
"closed" > Save;
detail < /span>
    < span;
var default_25 = /** @class */ (function () {
    function default_25() {
    }
    return default_25;
}());
"opened" >
    /** @class */ (function () {
        function class_21() {
        }
        return class_21;
    }());
"HorizontalContainer" > /** @class */ (function () {
    function class_22() {
    }
    return class_22;
}());
"HorizontalChild" > ;
/span><span class="HorizontalChild">⮝</span > /** @class */ (function () {
    function class_23() {
    }
    return class_23;
}());
"HorizontalChild" > ;
/span></div >
    /span>
    < /button>
    < /div>
    < div;
var default_26 = /** @class */ (function () {
    function default_26() {
    }
    return default_26;
}());
"detail measurable";
style = "display: none" >
    --left;
right;
eval;
r_;
str;
any / --add;
dataset;
to;
name;
input.
;
d_;
str;
any /
    _ / js;
any;
_rule;
$;
a$;
js;
any;
_export;
jq;
js;
any;
_chainFinal;
_chain;
_constraint;
size;
js;
bool;
inequality;
_dstyle / js -  > css;
str;
_import;
size;
js;
any;
-- >
    /** @class */ (function () {
        function class_24() {
        }
        return class_24;
    }());
"template measurable _root roww columnContainer" >
    /** @class */ (function () {
        function class_25() {
        }
        return class_25;
    }());
"deleteRule columnChild btn btn-danger" > X < /button>
    < div;
var default_27 = /** @class */ (function () {
    function default_27() {
    }
    return default_27;
}());
"columnChild" >
    /** @class */ (function () {
        function class_26() {
        }
        return class_26;
    }());
"roww hideOn r_ d_" >
    Name < /span>
    < span;
var default_28 = /** @class */ (function () {
    function default_28() {
    }
    return default_28;
}());
"nameprefix" > _ < /span>
    < input;
var default_29 = /** @class */ (function () {
    function default_29() {
    }
    return default_29;
}());
"fill name";
type = "text";
placeholder = "attribute name";
pattern = "[.]+" >
    /label>
    < /div>
    < label;
var default_30 = /** @class */ (function () {
    function default_30() {
    }
    return default_30;
}());
"expressionRoot" >
    /** @class */ (function () {
        function class_27() {
        }
        return class_27;
    }());
"leftside hideOn _ _dstyle _chain _chainFinal";
type = "text";
placeholder = "Left side" >
    /** @class */ (function () {
        function class_28() {
        }
        return class_28;
    }());
"operatortext hideOn _constraint _ _dstyle operator" >= /span>
    < select;
var default_31 = /** @class */ (function () {
    function default_31() {
    }
    return default_31;
}());
"hideOn _ _dstyle operator" > label;
"operator" >
    value;
"<=" >  & lt;
/option>
    < option;
value = "=" >= /option>
    < option;
value = ">=" >  & gt;
/option>
    < /optgroup></select >
    /** @class */ (function () {
        function class_29() {
        }
        return class_29;
    }());
"rightside hideon_chain _chainFinal ";
type = "text";
placeholder = "Right side" >
    /label>
    < div;
var default_32 = /** @class */ (function () {
    function default_32() {
    }
    return default_32;
}());
"roww hideOn r_ d_" >
    Relative;
target < /span>
    < input;
var default_33 = /** @class */ (function () {
    function default_33() {
    }
    return default_33;
}());
"fill relativetarget";
type = "text";
placeholder = "jQuery selector" >
    /label>
    < /div>
    < div;
var default_34 = /** @class */ (function () {
    function default_34() {
    }
    return default_34;
}());
"roww hideOn r_ d_" >
    Evaluation(debug) < /span>
    < input;
var default_35 = /** @class */ (function () {
    function default_35() {
    }
    return default_35;
}());
"fill evaluation";
type = "text";
placeholder = "empty string" >
    /label>
    < /div>
    < a;
var default_36 = /** @class */ (function () {
    function default_36() {
    }
    return default_36;
}());
"roww outputerror left" > /a>
    < a;
var default_37 = /** @class */ (function () {
    function default_37() {
    }
    return default_37;
}());
"roww outputerror right" > /a>
    < /div>
    < /div>
    < !--TEMPLATES;
END-- >
    /** @class */ (function () {
        function class_30() {
        }
        return class_30;
    }());
"addHeader" >
    /** @class */ (function () {
        function class_31() {
        }
        return class_31;
    }());
"btn btn-secondary addmeasurable" > Add < /button>
    < select;
var default_38 = /** @class */ (function () {
    function default_38() {
    }
    return default_38;
}());
"attributetypeadd" > label;
"Attribute type" >
    value;
"d_" > jQueryUI;
drag;
option < /option>
    < option;
value = "r_" > jQueryUI;
resize;
option < /option>
    < option;
value = "_rule" > _rule < /option>
    < option;
value = "_dstyle" > _dstyle < /option>
    < option;
value = "_export" > _export < /option>
    < option;
value = "_constraint" > _constraint < /option>
    < option;
value = "_import" > _import < /option>
    < option;
value = "_chain" > _chain < /option>
    < option;
value = "_chainFinal" > _chainFinal < /option>
    < option;
value = "_" > _(Pseudo - variable) < /option>
    < /optgroup></select >
    /h6>
    < h6 > _D < /h6>
    < div;
var default_39 = /** @class */ (function () {
    function default_39() {
    }
    return default_39;
}());
"d_Container" > /div>
    < h6 > _R < /h6>
    < div;
var default_40 = /** @class */ (function () {
    function default_40() {
    }
    return default_40;
}());
"r_Container" > /div>
    < h6 > _(Pseudo - variables) < /h6>
    < div;
var default_41 = /** @class */ (function () {
    function default_41() {
    }
    return default_41;
}());
"_Container" > /div>
    < h6 > _Rule < /h6>
    < div;
var default_42 = /** @class */ (function () {
    function default_42() {
    }
    return default_42;
}());
"_ruleContainer" > /div>
    < h6 > _Export < /h6>
    < div;
var default_43 = /** @class */ (function () {
    function default_43() {
    }
    return default_43;
}());
"_exportContainer" > /div>
    < h6 > _Constraint < /h6>
    < div;
var default_44 = /** @class */ (function () {
    function default_44() {
    }
    return default_44;
}());
"_constraintContainer" > /div>
    < h6 > _Dstyle < /h6>
    < div;
var default_45 = /** @class */ (function () {
    function default_45() {
    }
    return default_45;
}());
"_dstyleContainer" > /div>
    < h6 > _Import < /h6>
    < div;
var default_46 = /** @class */ (function () {
    function default_46() {
    }
    return default_46;
}());
"_importContainer" > /div>
    < h6 > _Chain < /h6>
    < div;
var default_47 = /** @class */ (function () {
    function default_47() {
    }
    return default_47;
}());
"_chainContainer" > /div>
    < h6 > _ChainFinal < /h6>
    < div;
var default_48 = /** @class */ (function () {
    function default_48() {
    }
    return default_48;
}());
"_chainFinalContainer" > /div>
    < /div>
    < div;
var default_49 = /** @class */ (function () {
    function default_49() {
    }
    return default_49;
}());
"detail save";
style = "display: none" >
    /** @class */ (function () {
        function class_32() {
        }
        return class_32;
    }());
"saveasrow" >
    /** @class */ (function () {
        function class_33() {
        }
        return class_33;
    }());
"delete btn btn-secondary" > Delete < /button>
    < button;
var default_50 = /** @class */ (function () {
    function default_50() {
    }
    return default_50;
}());
("saveas btn btn-secondary" > Save) / button >
    /** @class */ (function () {
        function class_34() {
        }
        return class_34;
    }());
"saveas form-control";
placeholder = "new style name" >
    /div>
    < h6 > Style;
is;
compatible;
with ()
    : /h6>
        < div;
var default_51 = /** @class */ (function () {
    function default_51() {
    }
    return default_51;
}());
"compatibleRow" >
    M2;
Elements < /span><input class="metamodel" type="checkbox"></label >
    M1;
Elements < /span><input class="model" type="checkbox"></label >
    /div>
    < h6 > And;
appliable;
to: /h6>
    < div;
var default_52 = /** @class */ (function () {
    function default_52() {
    }
    return default_52;
}());
"compatibleRow" >
    Class < /span><input class="class" type="checkbox"></label >
    Attributes < /span><input class="attribute" type="checkbox"></label >
    References < /span><input class="reference" type="checkbox"></label >
    Operations < /span><input class="operation" type="checkbox"></label >
    Parameters < /span><input class="parameter" type="checkbox"></label >
    /div>
    < /div>
    < div;
var default_53 = /** @class */ (function () {
    function default_53() {
    }
    return default_53;
}());
"separatorShell" >
    /** @class */ (function () {
        function class_35() {
        }
        return class_35;
    }());
"vertexStyle separator" >
    Preview & nbsp;
of & nbsp;
/span>
    < select;
var default_54 = /** @class */ (function () {
    function default_54() {
    }
    return default_54;
}());
"previewselector" > /select>
    < /label></h6 >
    /** @class */ (function () {
        function class_36() {
        }
        return class_36;
    }());
"helpButton vertex btn btn-info";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < !-- : ;
-- >
    /** @class */ (function () {
        function class_37() {
        }
        return class_37;
    }());
"style instances colContainer" >
    /** @class */ (function () {
        function class_38() {
        }
        return class_38;
    }());
"separatorShell top" >
    /** @class */ (function () {
        function class_39() {
        }
        return class_39;
    }());
"colChild" >
    /** @class */ (function () {
        function class_40() {
        }
        return class_40;
    }());
"roww" >
    /** @class */ (function () {
        function class_41() {
        }
        return class_41;
    }());
"vertexStyle separator htmllevel" > /h6>
    < /div>
    < div;
var default_55 = /** @class */ (function () {
    function default_55() {
    }
    return default_55;
}());
"colChild" >
    /** @class */ (function () {
        function class_42() {
        }
        return class_42;
    }());
"stylelabel" > Style;
/span>
    < select;
var default_56 = /** @class */ (function () {
    function default_56() {
    }
    return default_56;
}());
"stylename" > /select>
    < /div>
    < /label>
    < button;
var default_57 = /** @class */ (function () {
    function default_57() {
    }
    return default_57;
}());
"helpButton btn btn-info";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_58 = /** @class */ (function () {
    function default_58() {
    }
    return default_58;
}());
"html";
placeholder = "Html for $##name$ (m2 class)." > /div>
    < button;
var default_59 = /** @class */ (function () {
    function default_59() {
    }
    return default_59;
}());
"detail btn btn-primary" > Save;
detail < /button>
    < div;
var default_60 = /** @class */ (function () {
    function default_60() {
    }
    return default_60;
}());
"detail";
style = "display: none" >
    /** @class */ (function () {
        function class_43() {
        }
        return class_43;
    }());
"saveasrow" >
    /** @class */ (function () {
        function class_44() {
        }
        return class_44;
    }());
"delete btn btn-secondary" > Delete < /button>
    < button;
var default_61 = /** @class */ (function () {
    function default_61() {
    }
    return default_61;
}());
("saveas btn btn-secondary" > Save) / button >
    /** @class */ (function () {
        function class_45() {
        }
        return class_45;
    }());
"saveas form-control";
placeholder = "new style name" >
    /div>
    < h6 > Style;
is;
compatible;
with ()
    : /h6>
        < div;
var default_62 = /** @class */ (function () {
    function default_62() {
    }
    return default_62;
}());
"compatibleRow" >
    M2;
Elements < /span><input class="metamodel" type="checkbox"></label >
    M1;
Elements < /span><input class="model" type="checkbox"></label >
    /div>
    < h6 > And;
appliable;
to: /h6>
    < div;
var default_63 = /** @class */ (function () {
    function default_63() {
    }
    return default_63;
}());
"compatibleRow" >
    Class < /span><input class="class" type="checkbox"></label >
    Attributes < /span><input class="attribute" type="checkbox"></label >
    References < /span><input class="reference" type="checkbox"></label >
    Operations < /span><input class="operation" type="checkbox"></label >
    Parameters < /span><input class="parameter" type="checkbox"></label >
    /div>
    < /div>
    < div;
var default_64 = /** @class */ (function () {
    function default_64() {
    }
    return default_64;
}());
"separatorShell" >
    /** @class */ (function () {
        function class_46() {
        }
        return class_46;
    }());
"vertexStyle separator" >
    Preview & nbsp;
of & nbsp;
/span>
    < select;
var default_65 = /** @class */ (function () {
    function default_65() {
    }
    return default_65;
}());
"previewselector" > /select>
    < /label></h6 >
    /** @class */ (function () {
        function class_47() {
        }
        return class_47;
    }());
"helpButton vertex btn btn-info";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_66 = /** @class */ (function () {
    function default_66() {
    }
    return default_66;
}());
"preview" > /div>
    < /div>
    < /div>
    < div;
var default_67 = /** @class */ (function () {
    function default_67() {
    }
    return default_67;
}());
"Template vertex classe" >
    /** @class */ (function () {
        function class_48() {
        }
        return class_48;
    }());
"m2hide" >
    type;
"checkbox";
var default_68 = /** @class */ (function () {
    function default_68() {
    }
    return default_68;
}());
"showAsEdge" >
    /** @class */ (function () {
        function class_49() {
        }
        return class_49;
    }());
"showAsEdgeText" > ;
/span>
    < /label>
    < div;
var default_69 = /** @class */ (function () {
    function default_69() {
    }
    return default_69;
}());
"separatorShell top" >
    /** @class */ (function () {
        function class_50() {
        }
        return class_50;
    }());
"vertexStyle separator" > Own;
Html: /span></h6 >
    /** @class */ (function () {
        function class_51() {
        }
        return class_51;
    }());
"helpButton vertex";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_70 = /** @class */ (function () {
    function default_70() {
    }
    return default_70;
}());
"vertexStyle own html";
placeholder = "Html for $##name$ (m2 class)." > /div>
    < div;
var default_71 = /** @class */ (function () {
    function default_71() {
    }
    return default_71;
}());
"separatorShell" >
    /** @class */ (function () {
        function class_52() {
        }
        return class_52;
    }());
"vertexStyle separator" > Preview;
/span></h6 >
    /** @class */ (function () {
        function class_53() {
        }
        return class_53;
    }());
"helpButton vertex";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_72 = /** @class */ (function () {
    function default_72() {
    }
    return default_72;
}());
"vertexStyle own preview" > /div>
    < div;
var default_73 = /** @class */ (function () {
    function default_73() {
    }
    return default_73;
}());
"separatorShell top m1hide" >
    /** @class */ (function () {
        function class_54() {
        }
        return class_54;
    }());
"vertexStyle separator" > Instances & nbsp;
Html: /span></h6 >
    /** @class */ (function () {
        function class_55() {
        }
        return class_55;
    }());
"helpButton vertex";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_74 = /** @class */ (function () {
    function default_74() {
    }
    return default_74;
}());
"vertexStyle instances html m1hide";
placeholder = "Html for instances of $##name$ (m1 class)." > /div>
    < div;
var default_75 = /** @class */ (function () {
    function default_75() {
    }
    return default_75;
}());
"separatorShell m1hide" >
    /** @class */ (function () {
        function class_56() {
        }
        return class_56;
    }());
"vertexStyle separator" > Preview;
/span></h6 >
    /** @class */ (function () {
        function class_57() {
        }
        return class_57;
    }());
"helpButton vertex";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_76 = /** @class */ (function () {
    function default_76() {
    }
    return default_76;
}());
"vertexStyle instances preview m1hide" > /div>
    < /div>
    < div;
var default_77 = /** @class */ (function () {
    function default_77() {
    }
    return default_77;
}());
"Template attribute" >
    /** @class */ (function () {
        function class_58() {
        }
        return class_58;
    }());
"separatorShell top" >
    /** @class */ (function () {
        function class_59() {
        }
        return class_59;
    }());
"attributeStyle separator" > Own;
Html: /h6>
    < button;
var default_78 = /** @class */ (function () {
    function default_78() {
    }
    return default_78;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_79 = /** @class */ (function () {
    function default_79() {
    }
    return default_79;
}());
"attributeStyle own html";
placeholder = "Html for $##name$ (m2 attribute)." > /div>
    < div;
var default_80 = /** @class */ (function () {
    function default_80() {
    }
    return default_80;
}());
"separatorShell" >
    /** @class */ (function () {
        function class_60() {
        }
        return class_60;
    }());
"attributeStyle separator" > Preview;
/h6>
    < button;
var default_81 = /** @class */ (function () {
    function default_81() {
    }
    return default_81;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_82 = /** @class */ (function () {
    function default_82() {
    }
    return default_82;
}());
"attributeStyle own preview" > /div>
    < div;
var default_83 = /** @class */ (function () {
    function default_83() {
    }
    return default_83;
}());
"separatorShell top m1hide" >
    /** @class */ (function () {
        function class_61() {
        }
        return class_61;
    }());
"attributeStyle separator" > Instances;
Html: /h6>
    < button;
var default_84 = /** @class */ (function () {
    function default_84() {
    }
    return default_84;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_85 = /** @class */ (function () {
    function default_85() {
    }
    return default_85;
}());
"attributeStyle instances html m1hide";
placeholder = "Html for instances of $##name$ (m1 attribute)." > /div>
    < div;
var default_86 = /** @class */ (function () {
    function default_86() {
    }
    return default_86;
}());
"separatorShell m1hide" >
    /** @class */ (function () {
        function class_62() {
        }
        return class_62;
    }());
"attributeStyle separator" > Preview;
/h6>
    < button;
var default_87 = /** @class */ (function () {
    function default_87() {
    }
    return default_87;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_88 = /** @class */ (function () {
    function default_88() {
    }
    return default_88;
}());
"attributeStyle instances preview m1hide" > /div>
    < /div>
    < div;
var default_89 = /** @class */ (function () {
    function default_89() {
    }
    return default_89;
}());
"Template reference" >
    /** @class */ (function () {
        function class_63() {
        }
        return class_63;
    }());
"separatorShell top" >
    /** @class */ (function () {
        function class_64() {
        }
        return class_64;
    }());
"referenceStyle separator" > Own;
Html: /h6>
    < button;
var default_90 = /** @class */ (function () {
    function default_90() {
    }
    return default_90;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_91 = /** @class */ (function () {
    function default_91() {
    }
    return default_91;
}());
"referenceStyle own html";
placeholder = "Html for $##name$ (m2 reference)." > /div>
    < div;
var default_92 = /** @class */ (function () {
    function default_92() {
    }
    return default_92;
}());
"separatorShell" >
    /** @class */ (function () {
        function class_65() {
        }
        return class_65;
    }());
"referenceStyle separator" > Preview;
/h6>
    < button;
var default_93 = /** @class */ (function () {
    function default_93() {
    }
    return default_93;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_94 = /** @class */ (function () {
    function default_94() {
    }
    return default_94;
}());
"referenceStyle own preview" > /div>
    < div;
var default_95 = /** @class */ (function () {
    function default_95() {
    }
    return default_95;
}());
"separatorShell top m1hide" >
    /** @class */ (function () {
        function class_66() {
        }
        return class_66;
    }());
"referenceStyle separator" > Instances;
Html: /h6>
    < button;
var default_96 = /** @class */ (function () {
    function default_96() {
    }
    return default_96;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_97 = /** @class */ (function () {
    function default_97() {
    }
    return default_97;
}());
"referenceStyle instances html m1hide";
placeholder = "Html for instances of $##name$ (m1 reference)." > /div>
    < div;
var default_98 = /** @class */ (function () {
    function default_98() {
    }
    return default_98;
}());
"separatorShell m1hide" >
    /** @class */ (function () {
        function class_67() {
        }
        return class_67;
    }());
"referenceStyle separator" > Preview;
/h6>
    < button;
var default_99 = /** @class */ (function () {
    function default_99() {
    }
    return default_99;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_100 = /** @class */ (function () {
    function default_100() {
    }
    return default_100;
}());
"referenceStyle instances preview m1hide" > /div>
    < /div>
    < div;
var default_101 = /** @class */ (function () {
    function default_101() {
    }
    return default_101;
}());
"Template edge" >
    Edge;
style < /h1>
    < table > Style < /td>
    < td > /** @class */ (function () {
    function class_68() {
    }
    return class_68;
}());
"edgeStyle" > label;
"Edge style" >
    value;
"straight";
title = "./img/icons/edge/straight.jpg" > Straight < /option>
    < option;
value = "angular23Auto";
title = "./img/icons/edge/angular23.jpg" > Angulated(auto) < /option>
    < option;
value = "angular2";
title = "./img/icons/edge/angular2.jpg" > Angulated(2) < /option>
    < option;
value = "angular3";
title = "./img/icons/edge/angular3.jpg" > Angulated(3) < /option>
    < /optgroup></select > /td>
    < /tr>
    < tr > colspan;
"2";
var default_102 = /** @class */ (function () {
    function default_102() {
    }
    return default_102;
}());
"title" > On;
normal;
status < /td></tr >
    Color < /td>
    < td > type;
"color";
var default_103 = /** @class */ (function () {
    function default_103() {
    }
    return default_103;
}());
"edgeColor common" > /td>
    < /tr>
    < tr >
    Width < /td>
    < td > type;
"number";
var default_104 = /** @class */ (function () {
    function default_104() {
    }
    return default_104;
}());
"edgeWidth common";
min = "0";
step = "0.1";
placeholder = "0" > /td>
    < /tr>
    < tr > colspan;
"2";
var default_105 = /** @class */ (function () {
    function default_105() {
    }
    return default_105;
}());
"title" > On;
highlight;
preview < /td></tr >
    Color < /td>
    < td > type;
"color";
var default_106 = /** @class */ (function () {
    function default_106() {
    }
    return default_106;
}());
"edgeColor highlight" > /td>
    < /tr>
    < tr >
    Width < /td>
    < td > type;
"number";
var default_107 = /** @class */ (function () {
    function default_107() {
    }
    return default_107;
}());
"edgeWidth highlight";
min = "0";
step = "0.1";
placeholder = "0" > /td>
    < /tr>
    < tr > colspan;
"2";
var default_108 = /** @class */ (function () {
    function default_108() {
    }
    return default_108;
}());
"title" > On;
selection < /td></tr >
    Color < /td>
    < td > type;
"color";
var default_109 = /** @class */ (function () {
    function default_109() {
    }
    return default_109;
}());
"edgeColor selected" > /td>
    < /tr>
    < tr >
    Width < /td>
    < td > type;
"number";
var default_110 = /** @class */ (function () {
    function default_110() {
    }
    return default_110;
}());
"edgeWidth selected";
min = "0";
step = "0.1";
placeholder = "0" > /td>
    < /tr>
    < /tbody></table >
    EdgePoint;
style < /h1>
    < table > colspan;
"2";
var default_111 = /** @class */ (function () {
    function default_111() {
    }
    return default_111;
}());
"title" > On;
normal;
status < /td></tr >
    Radius < /td>
    < td > type;
"number";
var default_112 = /** @class */ (function () {
    function default_112() {
    }
    return default_112;
}());
"edgePoint radius";
min = "0";
step = "0.1";
placeholder = "0" > /td>
    < /tr>
    < tr >
    Border;
Width < /td>
    < td > type;
"number";
var default_113 = /** @class */ (function () {
    function default_113() {
    }
    return default_113;
}());
"edgePoint strokeW";
min = "0";
step = "0.1";
placeholder = "0" > /td>
    < /tr>
    < tr >
    Border;
Color < /td>
    < td > type;
"color";
var default_114 = /** @class */ (function () {
    function default_114() {
    }
    return default_114;
}());
"edgePoint stroke" > /td>
    < /tr>
    < tr >
    Fill;
Color < /td>
    < td > type;
"color";
var default_115 = /** @class */ (function () {
    function default_115() {
    }
    return default_115;
}());
"edgePoint fill" > /td>
    < /tr>
    < tr > colspan;
"2";
var default_116 = /** @class */ (function () {
    function default_116() {
    }
    return default_116;
}());
"title" > On;
bounding;
preview < /td></tr >
    Radius < /td>
    < td > type;
"number";
var default_117 = /** @class */ (function () {
    function default_117() {
    }
    return default_117;
}());
"edgePointPreview radius";
min = "0";
step = "0.1";
placeholder = "0" > /td>
    < /tr>
    < tr >
    Border;
Width < /td>
    < td > type;
"number";
var default_118 = /** @class */ (function () {
    function default_118() {
    }
    return default_118;
}());
"edgePointPreview strokeW";
min = "0";
step = "0.1";
placeholder = "0" > /td>
    < /tr>
    < tr >
    Border;
Color < /td>
    < td > type;
"color";
var default_119 = /** @class */ (function () {
    function default_119() {
    }
    return default_119;
}());
"edgePointPreview stroke" > /td>
    < /tr>
    < tr >
    Fill;
Color < /td>
    < td > type;
"color";
var default_120 = /** @class */ (function () {
    function default_120() {
    }
    return default_120;
}());
"edgePointPreview fill" > /td>
    < /tr>
    < tr > colspan;
"2";
var default_121 = /** @class */ (function () {
    function default_121() {
    }
    return default_121;
}());
"title" > While;
moving < /td></tr >
    Radius < /td>
    < td > type;
"number";
var default_122 = /** @class */ (function () {
    function default_122() {
    }
    return default_122;
}());
"edgePointSelected radius";
min = "0";
step = "0.1";
placeholder = "0" > /td>
    < /tr>
    < tr >
    Border;
Width < /td>
    < td > type;
"number";
var default_123 = /** @class */ (function () {
    function default_123() {
    }
    return default_123;
}());
"edgePointSelected strokeW";
min = "0";
step = "0.1";
placeholder = "0" > /td>
    < /tr>
    < tr >
    Border;
Color < /td>
    < td > type;
"color";
var default_124 = /** @class */ (function () {
    function default_124() {
    }
    return default_124;
}());
"edgePointSelected stroke" > /td>
    < /tr>
    < tr >
    Fill;
Color < /td>
    < td > type;
"color";
var default_125 = /** @class */ (function () {
    function default_125() {
    }
    return default_125;
}());
"edgePointSelected fill" > /td>
    < /tr>
    < /tbody></table >
    contenteditable;
"true";
var default_126 = /** @class */ (function () {
    function default_126() {
    }
    return default_126;
}());
"edgeStyle label html";
placeholder = "Label of link. Not filled means without label." > /div>
    < div;
var default_127 = /** @class */ (function () {
    function default_127() {
    }
    return default_127;
}());
"separatorShell" >
    /** @class */ (function () {
        function class_69() {
        }
        return class_69;
    }());
"edgeStyle separator" > Preview;
/h6>
    < button;
var default_128 = /** @class */ (function () {
    function default_128() {
    }
    return default_128;
}());
"helpButton edgeLabel";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_129 = /** @class */ (function () {
    function default_129() {
    }
    return default_129;
}());
"edgeStyle preview" > /div>
    < /div>
    < div;
var default_130 = /** @class */ (function () {
    function default_130() {
    }
    return default_130;
}());
"Template operation" >
    /** @class */ (function () {
        function class_70() {
        }
        return class_70;
    }());
"separatorShell top" >
    /** @class */ (function () {
        function class_71() {
        }
        return class_71;
    }());
"operationStyle separator" > Own;
Html: /h6>
    < button;
var default_131 = /** @class */ (function () {
    function default_131() {
    }
    return default_131;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_132 = /** @class */ (function () {
    function default_132() {
    }
    return default_132;
}());
"operationStyle own html";
placeholder = "Html for $##name$ (Operation)." > /div>
    < div;
var default_133 = /** @class */ (function () {
    function default_133() {
    }
    return default_133;
}());
"separatorShell" >
    /** @class */ (function () {
        function class_72() {
        }
        return class_72;
    }());
"operationStyle separator" > Preview;
/h6>
    < button;
var default_134 = /** @class */ (function () {
    function default_134() {
    }
    return default_134;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_135 = /** @class */ (function () {
    function default_135() {
    }
    return default_135;
}());
"operationStyle own preview" > /div>
    < div;
var default_136 = /** @class */ (function () {
    function default_136() {
    }
    return default_136;
}());
"separatorShell top m1hide" >
    /** @class */ (function () {
        function class_73() {
        }
        return class_73;
    }());
"operationStyle separator" > Instances;
Html: /h6>
    < button;
var default_137 = /** @class */ (function () {
    function default_137() {
    }
    return default_137;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_138 = /** @class */ (function () {
    function default_138() {
    }
    return default_138;
}());
"operationStyle instances html m1hide";
placeholder = "Html for instances of $##name$ (Operation, m1 view)." > /div>
    < div;
var default_139 = /** @class */ (function () {
    function default_139() {
    }
    return default_139;
}());
"separatorShell m1hide" >
    /** @class */ (function () {
        function class_74() {
        }
        return class_74;
    }());
"operationStyle separator" > Preview;
/h6>
    < button;
var default_140 = /** @class */ (function () {
    function default_140() {
    }
    return default_140;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_141 = /** @class */ (function () {
    function default_141() {
    }
    return default_141;
}());
"operationStyle instances preview m1hide" > /div>
    < /div>
    < div;
var default_142 = /** @class */ (function () {
    function default_142() {
    }
    return default_142;
}());
"Template parameter" >
    /** @class */ (function () {
        function class_75() {
        }
        return class_75;
    }());
"separatorShell top" >
    /** @class */ (function () {
        function class_76() {
        }
        return class_76;
    }());
"parameterStyle separator" > Own;
Html: /h6>
    < button;
var default_143 = /** @class */ (function () {
    function default_143() {
    }
    return default_143;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_144 = /** @class */ (function () {
    function default_144() {
    }
    return default_144;
}());
"parameterStyle own html";
placeholder = "Html for $##name$ (m2 parameter)." > /div>
    < div;
var default_145 = /** @class */ (function () {
    function default_145() {
    }
    return default_145;
}());
"separatorShell" >
    /** @class */ (function () {
        function class_77() {
        }
        return class_77;
    }());
"parameterStyle separator" > Preview;
/h6>
    < button;
var default_146 = /** @class */ (function () {
    function default_146() {
    }
    return default_146;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_147 = /** @class */ (function () {
    function default_147() {
    }
    return default_147;
}());
"parameterStyle own preview" > /div>
    < div;
var default_148 = /** @class */ (function () {
    function default_148() {
    }
    return default_148;
}());
"separatorShell top m1hide" >
    /** @class */ (function () {
        function class_78() {
        }
        return class_78;
    }());
"parameterStyle separator" > Instances;
Html: /h6>
    < button;
var default_149 = /** @class */ (function () {
    function default_149() {
    }
    return default_149;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
contenteditable = "true";
var default_150 = /** @class */ (function () {
    function default_150() {
    }
    return default_150;
}());
"parameterStyle instances html m1hide";
placeholder = "Html for instances of $##name$ (Parameter, m1 view)." > /div>
    < div;
var default_151 = /** @class */ (function () {
    function default_151() {
    }
    return default_151;
}());
"separatorShell m1hide" >
    /** @class */ (function () {
        function class_79() {
        }
        return class_79;
    }());
"parameterStyle separator" > Preview;
/h6>
    < button;
var default_152 = /** @class */ (function () {
    function default_152() {
    }
    return default_152;
}());
"helpButton field";
matTooltip = "todo: explain .EndPoint, .TypeSelector, .ClassSelector..." >  ? /button>
    < /div>
    < div : ;
var default_153 = /** @class */ (function () {
    function default_153() {
    }
    return default_153;
}());
"parameterStyle instances preview m1hide" > /div>
    < /div>
    < /div>
    < /div>;
//# sourceMappingURL=style-editor.component.html.js.map