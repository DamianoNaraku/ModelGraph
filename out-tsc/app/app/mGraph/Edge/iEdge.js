import { U, IVertex, EdgePoint, ModelPiece, Status, IReference, CursorFollowerEP, EdgePointFittizio, Point, GraphPoint, IClass } from '../../common/Joiner';
...
var IEdge = /** @class */ (function () {
    function IEdge(logic, startv, end) {
    ...
    IEdge.get = function (e) {
        // return ModelPiece.getLogic(e.m2target).edge;
    ...
    IEdge.generateContainmentHead = function () {
        var svg = IEdge.generateAggregationHead('white');
        return svg;
    };
    ...
    IEdge.prototype.refreshGui = function (debug, useRealEndVertex, usemidnodes) {
        ...
        this.appendTailHead(this.getEdgeHead(), false, pathStr);
        this.appendTailHead(this.getEdgeTail(), true, pathStr);
    ...
    IEdge.prototype.addEventListeners = function () {
        var $html = $(this.shell);
        $html.off('click.pbar').on('click.pbar', function (e) { return IVertex.ChangePropertyBarContentClick(e, true); });
    ...
    IEdge.prototype.getAllFakePoints = function (debug, pathStr) {
        if (debug === void 0) { debug = false; }
        if (pathStr === void 0) { pathStr = null; }
        // if (!this.html) { return null; }
        var d = pathStr ? pathStr : this.html.getAttributeNS(null, 'd');
        var dArr = d.split('L');
        if (dArr.length === 1) {
            dArr = [dArr[0], dArr[0]];
        }
        ...
        for (i = 0; i < dArr.length; i++) {
            var tmp = dArr[i].replace('M', '').replace('L', '').trim().split(' ');
            var pt = new GraphPoint(+tmp[0], +tmp[1]);
            var target = null;
            ...
            var fitToGrid = i !== 0 || i !== dArr.length - 1;
            ...
            if (i === dArr.length - 1 && !U.isOnHorizontalEdges(pt, this.end.getSize())) {
                fitHorizontal = false;
                fitVertical = true;
            }
            if (i === dArr.length - 1 && U.isOnHorizontalEdges(pt, this.end.getSize())) {
        ...
        if (puntiReali < 2 || puntiReali < realMidPoints.length) {
            ...
            U.pw(debug, 'fallimento nell\'assegnare fakepoints ai punti reali. Assegnati:' + puntiReali + ' / ' + prettyRealMidPoints.length
                + '; fittizi trovati:', prettyFittizi, ' reali:', prettyRealMidPoints, ', dArr:', dArr, ', path:', d);
        }
        return nodiFittizi;
    };
    ...
    IEdge.prototype.getBoundingMidPointsFake = function (e, style, canFail, arrFittizi) {
        ...
        // const edge: IEdge = ModelPiece.getLogic(e.m2target).edge;
    ...
    IEdge.prototype.getBoundingMidPointsStraight_OLD = function (e, canFail) {
        if (canFail === void 0) { canFail = false; }
        var edge = null; // ModelPiece.getLogic(e.m2target).edge;
        var clickedPt = GraphPoint.fromEvent(e);
    ...
    IEdge.prototype.onMouseOver = function (e, canFail, debug) {
        if (canFail === void 0) { canFail = false; }
        if (debug === void 0) { debug = false; }
        if (CursorFollowerEP.get().isAttached() || IEdge.refChanging) {
            return;
        }
    ...
    IEdge.prototype.onMouseDown = function (e) {
        ...
        CursorFollowerEP.get().attach(this, pos, this.midNodes.indexOf(tmp[0]));
    };
    ...
    IEdge.prototype.appendTailHead = function (cosa, onEnd, pathStr, debug) {
        ...
        if (onEnd) {
            ...
            startsub = pathStr.lastIndexOf('L', startsub);
            if (startsub === -1) {
                startsub = 0;
            }
        }
        ...
        var points = this.getAllFakePoints(false, pathStr);
        U.pe(points.length !== 2, 'expected exactly 2 points, the pathStr got substringed for that.');
        this.appendQualcosa(cosa, points[0].pos, points[1].pos);
    };
    IEdge.prototype.appendQualcosa = function (cosa, pt1, pt2real, debug) {
        if (debug === void 0) { debug = false; }
        var m = GraphPoint.getM(pt1, pt2real);
        var HeadSize = U.getSvgSize(this.edgeHead);
        // const HeadSize: GraphSize = this.owner.toGraphCoordS(U.sizeof(this.edgeHead));
        U.pif(false && debug, 'size of head: ', HeadSize);
        if (debug) {
            this.owner.markg(pt1, true);
            this.owner.markg(pt2real, false, 'green');
        }
        U.pif(false && debug, 'pt1:', pt1, 'pt2:', pt2real, 'm:', m);
        if (m === Number.POSITIVE_INFINITY) {
            ...
            cosa.setAttributeNS(null, 'transform', 'rotate(' + U.RadToDegree(degreeRad) + ' ' + center.x + ' ' + center.y + ')');
