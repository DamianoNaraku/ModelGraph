...
var ISidebar = /** @class */ (function () {
    ...
    ISidebar.prototype.sidebarNodeClick = function (e) {
        ...
        if (modelPiece instanceof IClass) {
            modelOfGraph.addEmptyClass(modelOfGraph.childrens[0]);
        }
        ...
        const pos = id.indexOf('sidebar_');
        U.pe( pos === -1, 'wrong click target or wrong id', e);
        const fullname = id.substr(pos + 'sidebar_'.length);
        const pkg = graphmodel.getPackage(fullname, false);
        const cla = graphmodel.getClass(fullname, false);
        U.pe( !pkg && !cla , 'the id does not match any class or package', e);
        const graph = metagraphmodel.graph;
        if (cla) { metagraphmodel.addEmptyClass(metagraphmodel.childrens[0] as IPackage); }
        if (pkg) { metagraphmodel.addEmptyP(); } */
    };
